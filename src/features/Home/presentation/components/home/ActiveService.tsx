...
export default function ActiveService() {
  ...
  const todayBooking = useMemo(() => {
    return activeBookings.find((b) => {
      const dateStr =
        b?.schedule?.startDateTime || b?.createdAt;
  ...
  const workerName =
    firstWorker?.fullName || "No worker assigned";
  ...
  const serviceName =
    serviceMap.get(String(booking?.serviceId)) ??
    "Service";
  ...
  const progress = useMemo(() => {
    ...
    return (
      progressMap[status as BookingStatus] ?? 20
    );
  }, [status]);
