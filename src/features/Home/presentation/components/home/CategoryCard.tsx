...
export default function CategoryCard({ category }: CategoryCardProps) {
  ...
  const goToCategory = () => navigate(`/services/${category._id}`);

  // Get localized category object from first service
  const localizedCategory = Object.values(
    category.services?.[0]?.category ?? {}
  )[0];

  const categoryName = localizedCategory?.name ?? category.name;
