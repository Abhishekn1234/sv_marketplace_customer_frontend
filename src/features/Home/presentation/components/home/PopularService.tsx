...
const PopularService: React.FC<Props> = ({ categories }) => {
  ...
  ? (
      category.services.reduce((sum, r) => sum + (r.avgRating || 0), 0) 
  ...
  return (
    <div
      ...
      className="relative bg-white rounded-2xl px-5 py-6 text-center border border-gray-200 cursor-pointer transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:shadow-2xl hover:-translate-y-1 hover:border-transparent active:scale-95 overflow-hidden group"
    >
     
      <span className="absolute top-0 left-0 right-0 h-[3px] bg-blue-600 scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100"></span>
