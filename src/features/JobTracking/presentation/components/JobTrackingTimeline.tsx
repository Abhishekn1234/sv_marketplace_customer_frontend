...
export default function JobTrackingTimeline() {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-7 border border-gray-200 shadow-sm w-full min-w-0">
      ...
      <div className="flex justify-between items-center mb-7">
        ...
        <div className="px-4 py-1 bg-emerald-100 text-emerald-600 text-xs font-semibold rounded-full break-words">
          {localBooking.status}
        </div>
      </div>
      ...
      >
        {/* Vertical Line */}
        <div className="absolute left-3 top-2 bottom-10 w-0.5 bg-gray-200"></div>
        ...
        {steps.map((step, idx) => {
          ...
          return (
            ...
            >
              {/* Dot */}
              <div
                className={`absolute left-0 top-1 w-6 h-6 rounded-full border-2 border-white shadow-sm ${dotClasses}`}
              />

              {/* Card */}
              <div
                className={`bg-gray-50 border rounded-xl p-4 w-full min-w-0 ${
                  step.status === "active"
              ...
              >
                ...
                <div className="text-xs text-gray-500 font-medium mb-2 break-words">
                  {step.time}
                </div>
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (!hasRated) {
                        setHasRated(true);
                        navigate(
                          `/servicerating/${localBooking._id}`
                        );
                      } else {
                        navigate("/bookings");
                      }
      ...
      {localBooking._id && (
        <PaymentModal
          ...
          serviceName={
            localBooking.service?.name ?? "Service Name"
          }
