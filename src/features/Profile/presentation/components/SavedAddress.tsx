...
export default function SavedAddress() {
  ...
  const EditIcon = () => (
    <svg className="w-4 h-4 text-gray-500 hover:text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
  ...
  const DeleteIcon = () => (
    <svg className="w-4 h-4 text-gray-500 hover:text-red-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
  ...
  const renderCard = (addr: any, index: number) => (
    <div key={addr.id} className="p-5 bg-gray-50">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex gap-2">
          {editingId === addr.id ? (
            <>
              <button onClick={saveEdit} className="px-3 py-1 bg-green-500 text-white rounded-lg text-xs">Save</button>
              <button onClick={cancelEdit} className="px-3 py-1 bg-gray-300 rounded-lg text-xs">Cancel</button>
            </>
          ) : (
            <>
              <button onClick={() => startEdit(addr.id, addr.value)} className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition">
                <EditIcon />
              </button>
              <button onClick={() => handleDelete(addr.id)} className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition">
                <DeleteIcon />
              </button>
            </>
          )}
      ...
      {editingId === addr.id
        ? renderInputWithSuggestions(tempAddress, setTempAddress, setTempAddress, `Enter ${addr.type} address`)
          ...
          <div className="p-5 bg-gray-50 border rounded-2xl space-y-4">
            <div className="flex gap-6">
              <label className="flex items-center gap-2 text-sm">
                <input type="radio" checked={selectedType === "home"} onChange={() => setSelectedType("home")} />
                Home
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="radio" checked={selectedType === "office"} onChange={() => setSelectedType("office")} />
                Office
              </label>
            </div>

            {renderInputWithSuggestions(newAddress, setNewAddress, setNewAddress, "Enter address")}

            <div className="flex gap-3">
              <button onClick={handleAdd} className="px-5 py-2 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition">
                Add Address
              </button>

              <button onClick={() => { setShowAddForm(false); setNewAddress(""); }} className="px-5 py-2 bg-gray-200 rounded-xl text-sm">
                Cancel
              </button>
            </div>
          </div>
        ) : (
          <button onClick={() => setShowAddForm(true)} className="w-full py-4 bg-blue-50 border-2 border-dashed border-blue-600 rounded-2xl text-blue-600 font-semibold hover:bg-blue-600 hover:text-white transition">
            + Add New Address
          </button>
        )}
      </div>
